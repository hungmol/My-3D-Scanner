...
//    cv::normalize(mask, mask_,255, 0, NORM_MINMAX, CV_8UC1);
...
//    float temp = 0.0f;
    ...
    for (int i = 0; i < height - 1; i++)
    {
        for (int j = 0; j < width - 1; j++)
        {
            ...
            quality.at<float>(Point(j, i)) = (sqdist(phi_center, phi_left) +
                    sqdist(phi_center, phi_right) +
                    sqdist(phi_center, phi_above) +
                    sqdist(phi_center, phi_behind)) / range.at<float>(Point(j, i));
        }
    }
